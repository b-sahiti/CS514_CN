    ...
    def getFlows(self):
        ...
        for p in range(0, k):
            for es in range(0, k / 2):
                ...
                for h in range(2, k / 2 + 2):
                    ...
                    suffixPriority = rand
                    # DOWN
                    cmd = "ovs-ofctl add-flow %s -O OpenFlow13 'table=0, idle_timeout=0,hard_timeout=0,priority=%s,arp,nw_dst=%s,actions=output:%s'" % (
                    switch, PrefixPriority, prefixIP, preficPort)
                    os.system(cmd)
                    cmd = "sudo ovs-ofctl add-flow %s -O OpenFlow13  'table=0, idle_timeout=0,hard_timeout=0,priority=%s,ip,nw_dst=%s,actions=output:%s'" % (
                    switch, PrefixPriority, prefixIP, preficPort)
                    os.system(cmd)
                # UP
                cmd = "ovs-ofctl add-group %s -O OpenFlow13 'group_id=5,type=select,bucket=output:%s,bucket=output:%s'" % (
                switch, 3, 4)
                os.system(cmd)
        ...
        for p in range(0, k):
            ...
            for ags in range(k / 2, k):
                ...
                # os.system(cmd)
                for es in range(0, k / 2):
                    prefixIP = str(10) + "." + str(p) + "." + str(es) + "." + str(0) + "/" + str(16)
                    preficPort = es + 1
                    PrefixPriority = 32768
                    ...
                    switch, PrefixPriority, prefixIP, preficPort)
                    os.system(cmd)
                    cmd = 'ovs-ofctl add-flow %s -O OpenFlow13 "table=0, idle_timeout=0,hard_timeout=0,priority=%s,ip,nw_dst=%s,actions=output:%s"' % (
                    switch, PrefixPriority, prefixIP, preficPort)
                    os.system(cmd)
                # up
                cmd = 'ovs-ofctl add-group %s -O OpenFlow13 "group_id=5,type=select,bucket=output:%s,bucket=output:%s"' % (
