...
class  staticFlows():
	def getFlows(self):
		...
		for p in range(0,4):
			for e in range(0,2):
				for h in range(2,4):
					ips.append("10."+str(p)+"."+str(e)+"."+str(h))
			...
			for es in range(0,k/2):
				...
				for h in range(2,k/2+2):					
					...
					for ip in ips:
						suffixIP=ip
						port=ip.split('.')[-1]
						suffixPort=int(port)+1
						cmd = "ovs-ofctl add-flow %s -O OpenFlow13 'table=0, idle_timeout=0,hard_timeout=0,priority=%s,arp,nw_dst=%s,actions=output:%s'" % (switch, suffixPriority, suffixIP, suffixPort)
						# print(cmd)
						os.system(cmd)
						cmd = "ovs-ofctl add-flow %s -O OpenFlow13 'table=0, idle_timeout=0,hard_timeout=0,priority=%s,ip,nw_dst=%s,actions=output:%s'" % (switch, suffixPriority, suffixIP, suffixPort)
						# print(cmd)
						os.system(cmd)
